<template>
  <nuxt-layout>
    ...
    <div class="source-settings w-full px-2">
      <div class="controls grid grid-cols-2 gap-2 gap-y-3 mb-4">
        ...
        <div>
          <input
            ...
            id="token"
            :value="token"
            @change="e => setToken((e.target as HTMLInputElement).value)"
          />
        </div>
        ...
        <div class="flex gap-4">
          <label for="kind_stream">
            <input
              ...
              name="kind"
              value="stream"
              @change="e => setStreamKind((e.target as HTMLInputElement).value)"
              :checked="kind === 'stream'"
              id="kind_stream"
            />
            Live stream
        ...
        <div>
          <input
            ...
            id="stream_id"
            :value="streamId"
            @change="e => setStreamId((e.target as HTMLInputElement).value)"
          />
        </div>
        <div></div>
        <div class="flex gap-2">
          <button
            @click="fetchSource"
            :disabled="!streamId || !token || pending"
          >
            Fetch
          </button>
      ...
      <pre class="stream-inf" v-if="streamInfo">{{
